...
function reactive_grasp(cost,matrix,vector_α,nb_iter,N_α)
	...
	@distributed for i in 1:nb_iter
		...
		if i % N_α == 0
			...
			x,z_glouton = greedy_randomized_construction(cost,matrix,vector_α[indice_α])
			x,z = simple_descent(cost,matrix,x,z_glouton)
			if z > z_best
				z_best = z
				x_max = x
			end
