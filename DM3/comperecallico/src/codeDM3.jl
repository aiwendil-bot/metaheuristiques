...
function indice_meilleure_evaluation(cost,liaisons_contraintes, liaisons_variables,subset_restant,cost_restant)
	evaluations = zeros(Float64,length(cost))
	somme::Int64 = 0
	for i in 1:length(cost)
		somme = 0
		for j in 1:length(liaisons_variables[i])
			somme += cost_restant[liaisons_variables[i][j]]
		end
